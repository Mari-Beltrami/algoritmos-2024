...
class Grafo:
    ...
    def insertar_vertice_con_datos(self, valor, otros_datos=None):
        """Inserta un nuevo vértice en el grafo con datos adicionales opcionales."""
        nodo = {
            'valor': valor,
            'aristas': [],  # Lista de aristas para este nodo
            'visitado': False,  # Marca de visitado para recorridos
            'otros_datos': otros_datos  # Información adicional del vértice
        }
        self.elementos.append(nodo)

    def obtener_datos_vertice(self, valor):
        """Obtiene los datos adicionales de un vértice si existen."""
        pos_vertice = self.buscar(valor)
        if pos_vertice is not None:
            return self.elementos[pos_vertice].get('otros_datos', None)
        return None

    def kruskal_con_filtro(self, filtro_tipo):
        """Aplica el algoritmo de Kruskal para encontrar el árbol de expansión mínima
        solo para los vértices que tienen el tipo especificado en 'otros_datos'."""
        def buscar_en_bosque(bosque, buscado):
            for conjunto in bosque:
                if buscado in bosque[conjunto]:
                    return conjunto
            return None
        ...
        # Filtrar nodos por tipo y preparar el bosque inicial
        for nodo in self.elementos:
            if nodo.get('otros_datos', {}).get('tipo') == filtro_tipo:
                bosque[nodo['valor']] = {nodo['valor']}
                for adyacente in nodo['aristas']:
                    # Solo agregar aristas cuyo destino también tenga el tipo correcto
                    destino_nodo = next((n for n in self.elementos if n['valor'] == adyacente['valor']), None)
                    if destino_nodo and destino_nodo.get('otros_datos', {}).get('tipo') == filtro_tipo:
                        aristas.arrive([nodo['valor'], adyacente['valor']], adyacente['peso'])
    
        resultado = []
        while len(bosque) > 1 and len(aristas.elements) > 0:
            arista = aristas.atention()
            origen_conjunto = buscar_en_bosque(bosque, arista[1][0])
            destino_conjunto = buscar_en_bosque(bosque, arista[1][1])
    
            # Solo unir conjuntos diferentes para evitar ciclos
            if origen_conjunto is not None and destino_conjunto is not None and origen_conjunto != destino_conjunto:
                resultado.append(f"{arista[1][0]} - {arista[1][1]} ({arista[0]} km)")
    
                # Unir los conjuntos en el bosque
                bosque[origen_conjunto].update(bosque[destino_conjunto])
                del bosque[destino_conjunto]
